 ...
 *
 * Message IN:
 *   { type: 'load',       model: string }                          — pre-load model
 *   { type: 'transcribe', audioData: Float32Array,
 *     chunkIndex: number, totalChunks: number }                    — one 30s chunk
 *
 * Message OUT:
...
self.onmessage = async (event: MessageEvent) => {
  ...
  if (msg.type === 'transcribe') {
    ...
    try {
      ...
      // Shift timestamps by the chunk's offset in the full audio
      ...
      const lines = chunksToSubtitleLines(shifted);
