 ...
 * transcribeWorker.ts
 *
 * ES-module Web Worker bundled by Vite.
 * @xenova/transformers is imported from npm (not CDN) so it is bundled
 * at build time. vite.config.ts sets worker esbuild target to 'esnext'
 * so BigInt literals inside the library pass through untransformed.
 *
 * Message IN:
...
// @ts-ignore — @xenova/transformers ships its own types; tsc skips with skipLibCheck
